// Hemline Market — app.js (with VISIBLE TEST BANNER)
(function () {
  // ------- Toast UI -------
  let toastBox;
  ...
  function normalizeSupabaseError(err) {
    try {
      ...
      if (m.includes('too many') || m.includes('rate')) return 'Slow down a sec—too many attempts.';
      if (m.includes('row level security') || m.includes('rls')) return 'You don’t have permission for that action.';
      if (m.includes('not null') || m.includes('missing')) return 'Required info is missing.';
      if (m.includes('unique') || m.includes('duplicate')) return 'Already submitted.';
      if (m.includes('invalid') || m.includes('bad input')) return 'Please check the values.';
...
`;
  ...
  // ------- VISIBLE TEST BANNER on /listings.html -------
  (function showTestBanner(){
    const path = (location.pathname || '').toLowerCase();
    if (!(path.endsWith('/listings.html') || path === '/listings.html')) return;

    const banner = document.createElement('div');
    banner.textContent = 'TEST BANNER — app.js is active';
    banner.style.cssText = 'position:fixed;left:0;right:0;top:0;z-index:99999;background:#dc2626;color:#fff;padding:10px 14px;font:14px system-ui';
