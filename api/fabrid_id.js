// api/fabric/[id].js
// Server-renders a listing as a full HTML page.
// URL: /fabric/abc-123  → indexable by Google with real title, description, price, images
// The existing listing.html still works for the interactive buy flow.

const SUPABASE_URL = "https://clkizksbvxjkoatdajgd.supabase.co";
const SUPABASE_ANON = process.env.SUPABASE_ANON_KEY || "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNsa2l6a3Nidnhqa29hdGRhamdkIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTQ2ODAyMDUsImV4cCI6MjA3MDI1NjIwNX0.m3wd6UAuqxa7BpcQof9mmzd8zdsmadwGDO0b7-nyBjI";
...
function buildDetails(listing) {
  const parts = [];
  if (listing.yardage) parts.push(`${listing.yardage} yards`);
  if (listing.width) parts.push(`${listing.width}" wide`);
  if (listing.fiber_content) parts.push(listing.fiber_content);
  if (listing.fabric_type) parts.push(listing.fabric_type);
  return parts.join(" · ");
}
...
export default async function handler(req, res) {
  const { id } = req.query;
  if (!id) return res.status(400).send("Invalid listing ID");
  ...
  const metaDescription = [
    ...
    listing.description ? listing.description.slice(0, 100) : "",
  ]
    .filter(Boolean)
    .join(" — ")
    .slice(0, 160);
  ...
  const jsonLd = JSON.stringify({
    ...
    ...(listing.yardage
      ? {
          additionalProperty: [
            { "@type": "PropertyValue", name: "Yards Available", value: listing.yardage },
            ...(listing.width
              ? [{ "@type": "PropertyValue", name: "Width", value: `${listing.width} inches` }]
              : []),
          ],
        }
      : {}),
  ...
  const imageGallery = images
    .map(
      (url, i) =>
        `<img src="${escapeHtml(url)}" alt="${escapeHtml(title)}${i > 0 ? ` — photo ${i + 1}` : ""}" loading="${i === 0 ? "eager" : "lazy"}" class="listing-photo"/>`
    )
    .join("\n");
...
<head>
  ...
  <style>
    ...
    .fabric-price { font-size: 28px; font-weight: 700; color: #1a1a1a; }
    .fabric-details { font-size: 14px; color: #666; }
    .fabric-seller { font-size: 13px; color: #888; }
    .fabric-seller strong { color: #444; }
    .fabric-description { font-size: 15px; line-height: 1.7; color: #333; white-space: pre-wrap; }
    .sold-badge { display: inline-block; background: #fee2e2; color: #991b1b; font-size: 12px; font-weight: 700; padding: 4px 12px; border-radius: 20px; text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    .fabric-cta-sold { background: #ccc; cursor: not-allowed; }
  </style>
</head>
<body>
  ...
  <main>
    <div class="fabric-page" itemscope itemtype="https://schema.org/Product">
      ...
      <div class="fabric-gallery">
        ${imageGallery || `<div class="listing-photo" style="background:#f5f0ea;display:flex;align-items:center;justify-content:center;color:#ccc;">No photo</div>`}
      </div>

      <div class="fabric-info">
        ${isSold ? '<span class="sold-badge">Sold</span>' : ""}
        <h1 class="fabric-title" itemprop="name">${escapeHtml(title)}</h1>
        ${price ? `<p class="fabric-price" itemprop="price" content="${(listing.price_cents / 100).toFixed(2)}">${escapeHtml(price)}</p>` : ""}
        ${details ? `<p class="fabric-details">${escapeHtml(details)}</p>` : ""}
        <p class="fabric-seller">Sold by <strong itemprop="seller">${escapeHtml(sellerName)}</strong></p>
        ${
          listing.description
            ? `<div class="fabric-description" itemprop="description">${escapeHtml(listing.description)}</div>`
            : ""
        }
        <a
          href="/listing.html?id=${encodeURIComponent(id)}"
          class="fabric-cta${isSold ? " fabric-cta-sold" : ""}"
