...
// Uses Resend's API via fetch (no SDK). In Step 2 we'll set RESEND_API_KEY in Vercel.
...
module.exports = async (req, res) => {
  ...
  try {
    // Expecting JSON from the frontend (we'll wire this in the next step)
...
`;
    ...
    const r = await fetch(RESEND_ENDPOINT, {
      ...
      body: JSON.stringify({
        // Use Resend's safe default sender so you don't need domain verification yet.
        // We can switch this to hello@hemlinemarket.com as the "from" after domain auth.
    ...
    if (!r.ok) {
      // Surface the Resend error to help debugging
